...
import type { Client, AppState, Game, Status, TaskSet, IncomeEntry } from './types'
...
// HELPERS
function statusClass(s: Status): string { return { Pending: 'status-pending', 'In Progress': 'status-inprog', Done: 'status-done', 'On Hold': 'status-hold' }[s] }
function progressColor(p: number): string { if (p >= 100) return '#4CAF50'; if (p >= 60) return '#4FC3F7'; if (p >= 30) return '#FFB74D'; return '#EF5350' }
function taskDots(tasks: TaskSet): string { return Object.entries(tasks).map(([k, v]) => `<span class="task-dot ${v ? 'dot-on' : 'dot-off'}" title="${TASK_LABELS[k]}">${v ? '●' : '○'}</span>`).join('') }
...
function formatTHB(amount: number): string { return new Intl.NumberFormat('th-TH', { style: 'currency', currency: 'THB', minimumFractionDigits: 0 }).format(amount) }
function calcStats() {
...
function renderAdminStats(): string {
  ...
  return `
    ...
    <div class="stats-panels">
      <div class="stats-panel"><h3>By Game</h3>${Object.entries(s.byGame).map(([g, c]) => `<div class="stat-game-row"><div class="stat-game-label"><img src="${GAMES[g as Game].logo}" style="width:16px; border-radius:2px; vertical-align:middle;" onerror="this.style.display='none'; this.nextElementSibling.style.display='inline';"><span class="game-emoji fallback-emoji" style="display:none;">${GAMES[g as Game].emoji}</span> ${g}</div><div class="stat-bar-bg"><div class="stat-bar-fill" style="width: ${s.total ? Math.round(c / s.total * 100) : 0}%; background-color: ${GAMES[g as Game].accent};"></div></div><div class="stat-count">${c}</div></div>`).join('')}</div>
...
async function loadPublicQueue() {
  ...
  try {
    ...
    if (ts && queueData.lastUpdated) ts.textContent = new Date(queueData.lastUpdated).toLocaleString('en-GB')
  } catch (err) {
...
function attachEventListeners() {
  document.querySelectorAll('.tab-btn').forEach(btn => {
    btn.addEventListener('click', (e) => { const tab = (e.target as HTMLElement).getAttribute('data-tab'); if (tab) { adminTab = tab as any; router() } })
  })

  document.querySelectorAll('[data-filter-game]').forEach(btn => { btn.addEventListener('click', (e) => { filterGame = (e.target as HTMLElement).getAttribute('data-filter-game') as any; router() }) })
  document.querySelectorAll('[data-filter-status]').forEach(btn => { btn.addEventListener('click', (e) => { filterStatus = (e.target as HTMLElement).getAttribute('data-filter-status') as any; router() }) })
  ...
  document.getElementById('lookupSearch')?.addEventListener('input', (e) => { 
    publicSearchQuery = (e.target as HTMLInputElement).value.toLowerCase()
    let count = 0
    document.querySelectorAll('.lookup-item').forEach(item => {
      const text = item.getAttribute('data-search-text') || ''
      if (text.includes(publicSearchQuery)) {
        (item as HTMLElement).style.display = 'flex'; count++
      } else {
        (item as HTMLElement).style.display = 'none'
      }
    })
    const hdr = document.getElementById('searchCountHeader')
    if (hdr) hdr.textContent = `Search Results (${count})`
  })
  ...
  document.querySelectorAll('.delete-client-btn').forEach(btn => {
    btn.addEventListener('click', (e: Event) => {
      if (confirm('Are you sure you want to delete this client?')) {
        state.clients = state.clients.filter(c => c.id !== (e.currentTarget as HTMLElement).getAttribute('data-client-id'))
        saveState(state); router()
      }
    })
  })
  ...
  document.querySelectorAll('.edit-income-btn').forEach(btn => {
    btn.addEventListener('click', (e: Event) => {
      const id = (e.currentTarget as HTMLElement).getAttribute('data-income-id')
      const entry = state.income.find(inc => inc.id === id)
      if (entry) showIncomeModal(entry)
    })
  })
  document.querySelectorAll('.delete-income-btn').forEach(btn => {
    btn.addEventListener('click', (e: Event) => {
      if (confirm('Are you sure you want to delete this income record?')) {
        state.income = state.income.filter(inc => inc.id !== (e.currentTarget as HTMLElement).getAttribute('data-income-id'))
        saveState(state); router()
      }
    })
  })
  ...
  document.getElementById('testTokenBtn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (user.username !== 'serensz') throw new Error('Unauthorized: This dashboard is restricted.')
...
function showIncomeModal(entry?: IncomeEntry) {
  const e: IncomeEntry = entry || { id: uid(), clientId: '', clientName: '', game: 'WuWa', service: '', amount: 0, paid: false, month: new Date().toISOString().slice(0, 7), notes: '' }
    ...
    <div class="modal-overlay" id="incomeModal">
      ...
      <div class="modal-body">
        <div class="form-grid">
          ...
          <div class="form-group"><label>Game *</label><select id="i_game">${(['WuWa', 'HSR', 'ZZZ', 'Endfield'] as Game[]).map(g => `<option value="${g}" ${e.game === g ? 'selected' : ''}>${GAMES[g].emoji} ${g}</option>`).join('')}</select></div>
  ...
  document.getElementById('saveIncomeBtn')?.addEventListener('click', () => {
    const clientName = (document.getElementById('i_clientName') as HTMLInputElement)?.value.trim()
    if (!clientName) { alert('Client Name required'); return }
    const updated: IncomeEntry = {
      id: entry?.id || uid(),
      clientId: e.clientId,
      clientName,
      game: (document.getElementById('i_game') as HTMLSelectElement)?.value as Game,
      service: (document.getElementById('i_service') as HTMLInputElement)?.value,
      amount: parseFloat((document.getElementById('i_amount') as HTMLInputElement)?.value || '0'),
      paid: (document.getElementById('i_paid') as HTMLInputElement)?.checked || false,
      month: (document.getElementById('i_month') as HTMLInputElement)?.value,
      notes: (document.getElementById('i_notes') as HTMLInputElement)?.value,
    }
    if (entry) state.income = state.income.map(inc => inc.id === updated.id ? updated : inc)
    else state.income.push(updated)
...
async function init() {
  ...
  if (config?.token) {
    ...
    } catch (e) { console.warn('GitHub session could not be restored automatically.') }
  }
  router()
}
