...
export function renderPublicLookup(clients, selectedClient, searchQuery, isLoading) {
    return `
...
export function renderPublicQueue(clients, isLoading, error) {
...
function renderClientDetailCard(client) {
    return `
...
function renderTaskDots(tasks) { return Object.entries(tasks).map(([_, v]) => `<span class="task-dot ${v ? 'dot-on' : 'dot-off'}">${v ? '●' : '○'}</span>`).join(''); }
...
function daysLeftValue(deadline) { if (!deadline)
    return '—'; const diff = Math.ceil((new Date(deadline).getTime() - Date.now()) / 86400000); if (diff < 0)
    return `${Math.abs(diff)}d overdue`; if (diff === 0)
    return 'Due today'; return `${diff}d left`; }
function getDeadlineClass(deadline) { if (!deadline)
    return ''; const diff = Math.ceil((new Date(deadline).getTime() - Date.now()) / 86400000); if (diff < 0)
    return 'overdue'; if (diff === 0)
    return 'due-today'; if (diff <= 3)
    return 'due-soon'; return 'due-ok'; }
