...
async function renderPublicPage(route) {
    ...
    if (route.page === 'public-home')
        ...
        pageContent = renderPublicLookup(publicClients, null, publicSearchQuery, publicLoading);
...
function renderAdminStats() {
    ...
    <div class="stats-panels">
      <div class="stats-panel"><h3>By Game</h3>${Object.entries(s.byGame).map(([g, c]) => `<div class="stat-game-row"><div class="stat-game-label">${GAMES[g].emoji} ${g}</div><div class="stat-bar-bg"><div class="stat-bar-fill" style="width: ${s.total ? Math.round(c / s.total * 100) : 0}%; background-color: ${GAMES[g].accent};"></div></div><div class="stat-count">${c}</div></div>`).join('')}</div>
...
async function loadPublicQueue() {
    ...
    try {
        const config = loadGistConfig();
        if (!config?.gistId)
            throw new Error('Queue not published yet');
...
function attachEventListeners() {
    ...
    document.querySelectorAll('[data-filter-game]').forEach(btn => {
        btn.addEventListener('click', (e) => { filterGame = e.target.getAttribute('data-filter-game'); router(); });
    });
    document.querySelectorAll('[data-filter-status]').forEach(btn => {
        btn.addEventListener('click', (e) => { filterStatus = e.target.getAttribute('data-filter-status'); router(); });
    });
    // Search
    document.getElementById('searchClients')?.addEventListener('input', (e) => { searchQuery = e.target.value; router(); });
    document.getElementById('lookupSearch')?.addEventListener('input', (e) => { publicSearchQuery = e.target.value; router(); });
    // Admin buttons
